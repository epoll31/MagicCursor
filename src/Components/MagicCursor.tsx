...
export function MagicCursorProvider(props: {
...
export function MagicCursor() {
  ...
  const className = twMerge(
    "fixed -translate-x-1/2 -translate-y-1/2 transition-transform duration-150 -z-10",
  ...
  return <div className={className} style={style} aria-disabled></div>;
}
...
export function CursorLock({
  ...
  const { lockClassName, lockStyle, noLock, holdLock, ...props } = rest;
  const handleEnter = () => {
    if (!noLock) {
      state.setState({
        ...state.state,
        ...
        lockAt: {
          x:
            (ref.current?.offsetLeft || 0) +
              (ref.current?.offsetWidth || 0) / 2 || 0,
          y:
            (ref.current?.offsetTop || 0) +
              (ref.current?.offsetHeight || 0) / 2 || 0,
        },
      });
    }
